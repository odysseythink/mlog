...
func TestHeader(t *testing.T) {
	...
	Info("testHeader")
	var line int
	format := "I0102 15:04:05.067890 %7d mlog_test.go:%d] testHeader\n"
	var gotPID int64
...
func TestVmoduleOn(t *testing.T) {
	setFlags()
	defer sinks.file.swap(sinks.file.newBuffers())
	if err := flag.Lookup("vmodule").Value.Set("mlog_test=2"); err != nil {
...
func TestVDepth(t *testing.T) {
	setFlags()
	defer sinks.file.swap(sinks.file.newBuffers())
	if err := flag.Lookup("vmodule").Value.Set("mlog_test=3"); err != nil {
		t.Fatalf("Failed to set -vmodule=mlog_test=3: %v", err)
	}
	defer flag.Lookup("vmodule").Value.Set("")
	...
	// Since vmodule is set to mlog_test=3, V(3) is true only for frames in
	// mlog_test. runInAnotherModule's stack frame is in log_vmodule_test, whereas
	// this test and the provided closures are in mlog_test. Therefore VDepth(0, 3)
...
var vGlobs = map[string]bool{
	...
	"mlog_test=1": false, // If -vmodule sets V to 1, V(2) will fail.
	"mlog_test=2": true,
	"mlog_test=3": true, // If -vmodule sets V to 1, V(3) will succeed.
...
func TestVmoduleFullGlobMultipleDirectories(t *testing.T) {
	...
	testVmoduleGlob(filepath.Join(dir, "*/mlog_test=2"), true, t)
	testVmoduleGlob(filepath.Join(dir, "*/mlog_????=2"), true, t)
}
...
func TestLogBacktraceAt(t *testing.T) {
	...
	if numAppearances < 2 {
		...
		//   ...
		//   .../mlog/mlog_test.go:280 (0x41ba91)
		//   ...
